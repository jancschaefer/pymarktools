...
class DeadImageChecker(AsyncChecker[ImageInfo]):
    ...
    async def check_urls_async(self, urls: list[str]) -> dict[str, dict[str, Any]]:
        ...
        method_is_overridden = (
            self.__class__.check_url is not DeadImageChecker.check_url or  # Subclassed
            (hasattr(self.check_url, '__name__') and 
             self.check_url.__name__ != 'check_url') or                    # Renamed function
            str(type(self.check_url)) == "<class 'function'>"              # Standalone function
        )
    ...
    def check_directory(
        ...
        return cast(
            dict[Path, list[ImageInfo]],
            self.run_async_with_fallback(
                self.check_directory_async, directory, include_pattern, exclude_pattern
            ),
        )
