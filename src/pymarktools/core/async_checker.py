...
class AsyncChecker(Generic[T]):
    ...
    async def discover_files_async(
        ...
        async def process_directory_level(dir_path: Path) -> list[Path]:
            ...
            try:
                ...
                file_tasks: list[asyncio.Task[Optional[Path]]] = [asyncio.create_task(check_and_add_file(file_path)) for file_path in files]
                file_results: list[Optional[Path] | BaseException] = await asyncio.gather(*file_tasks, return_exceptions=True)
    ...
    async def process_files_async(
        ...
        if files:
            tasks: list[asyncio.Task[tuple[Path, Any]]] = [asyncio.create_task(process_file_safe(file_path)) for file_path in files]
    ...
    async def process_items_async(
        ...
        try:
            ...
            tasks: list[asyncio.Task[tuple[str, Any]]] = [asyncio.create_task(process_with_semaphore(item)) for item in items]
