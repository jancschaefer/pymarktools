...
uvx pymarktools check README.md
```
...
pymarktools --verbose check README.md
pymarktools --quiet check docs/
```

Replace `<path>` with the specific file or directory you want to check.

### Check for Dead Links and Images

The `check` command validates both links and images by default:

```bash
# Check everything in current directory (both links and images)
pymarktools check

# Check specific path (both links and images)
pymarktools check docs/

# Check specific file
pymarktools check README.md

# Check only dead links, skip images
pymarktools check --no-check-dead-images

# Check only dead images, skip links
pymarktools check --no-check-dead-links

# Check with custom timeout and external validation
pymarktools check docs/ --timeout 60 --check-external

# Check only local files, skip external URLs
pymarktools check docs/ --no-check-external

# Check with pattern filtering
pymarktools check docs/ --include "*.md" --exclude "draft_*"
```

### Flexible Option Placement

Options can be specified at the callback level or at the command level:

```bash
# Options at callback level (apply to the entire check operation)
pymarktools check --timeout 30 --no-check-external docs/

# Mixed approach (command options override callback when both specified)
pymarktools check --include "*.md" --timeout 60 docs/
```
...
pymarktools check docs/ --no-check-local
...
pymarktools check <path>

# Disable external URL checking
pymarktools check <path> --no-check-external

# Automatically fix permanent redirects in source files
pymarktools check <path> --fix-redirects

# Custom timeout for HTTP requests
pymarktools check <path> --timeout 60
```
...
pymarktools check docs/ --include "*.md"

# Exclude draft files
pymarktools check docs/ --exclude "draft_*"

# Combine include and exclude patterns
pymarktools check docs/ --include "*.md" --exclude "temp_*"

# Multiple patterns work with fnmatch
pymarktools check assets/ --include "*.{jpg,png,gif}" --no-check-dead-links
```
...
```bash
pymarktools check <path> --no-follow-gitignore
```
...
pymarktools check docs/ --parallel

# Customize the number of async workers
pymarktools check docs/ --workers 8

# Disable async processing for sequential operation
pymarktools check docs/ --no-parallel
```
...
pymarktools --color check docs/

# Disable colored output for plain text
pymarktools --no-color check docs/

# Color output with status indicators
pymarktools check docs/  # Shows green ✓ for valid, red ✗ for broken
```
...
pymarktools --verbose check docs/
```
...
pymarktools check docs/
if [ $? -eq 0 ]; then
    echo "All links and images are valid!"
else
    echo "Found broken links or images, failing build"
    exit 1
fi
```
...
```bash
# Check current directory for dead links and images
pymarktools check
...
pymarktools check docs/ --fix-redirects --timeout 60

# Check images only with custom patterns
pymarktools check assets/ --include "*.{md,mdx}" --no-check-dead-links

# Check links only with mixed options
pymarktools check docs/ --timeout 30 --check-external --fix-redirects --no-check-dead-images
```
...
```bash
# Minimal CI check
pymarktools --quiet check docs/ --no-check-external

# Full CI validation
pymarktools check . --include "*.md" --timeout 30 || exit 1
```
